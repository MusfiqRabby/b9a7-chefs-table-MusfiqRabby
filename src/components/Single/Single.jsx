...
const Single = ({ single, handleToBtn }) => {
  ...
  return (
    <div class="card bg-base-100 shadow-xl">
      <figure>
        <img className=" w-full h-56 p-4 " src={recipe_image} />
      </figure>
      <div class="card-body">
        <h2 class="text-2xl font-semibold ">{recipe_name}</h2>
        <p className="text-[#878787]">{short_description}</p>
        <div>
          <h5 className="text-lg text-[#282828] font-medium">
          ...
          <div>
            {ingredients.map((ingre, idx) => (
              <div key={idx}>
                <li item={ingre} className="opacity-80">
                  {ingre}
                </li>
              </div>
            ))}
        ...
        <div class="mt-5">
          <button
            ...
            class="btn bg-[#0BE58A] rounded-full"
          >
            Want to Cook
          </button>
        </div>
